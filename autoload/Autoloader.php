 ...
 * @author Gordon McVey
 * @category Reefknot
 * @package gordian\reefknot\autoload
 */
class Autoloader implements iface\Autoloader
{
	 ...
	 * PHP >= 5.3 then you will normally want to use the \ character as the
	 * namespace separator.  If you're working with legacy code that uses PEAR
	 * style pseudo-namespacing then you will probably want to change this to
	 * the _ character, or to whatever convention you've used in your code.
	 *
	 * @var string
	 */
	private	$namespaceSep	= '';

	/**
	 * The namespace segment that should be considered the "root".  The
	 * autoloader will only attempt to load resources within the root namespace
	 *
	 * @var string
	 */
	...
	/**
	 * The root directory to search for resources in.  Each sub-namespace maps
	 * to a subdirectory of this root directory.
	 *
	 * @var string
	 */
	private	$pathRoot		= '';
	...
	/**
	 * Flag to determine if this instance of the autoloader should attempt to
	 * load or whether it should just be skipped.
	 *
	 * @var boolean
	 */
	 ...
	 * to temporarily skip a particular autoloader, but don't want to change
	...
	public function unregister ()
	{
		if (true === $this -> isRegistered ())
		{
			...
			if (true === $this -> isRegistered ())
			{
				throw new \RuntimeException ('Failed to unregister autoloader');
			}
		}
	 ...
	 * a class and will simply returns false immediately.  This means that
	 ...
	 *
	 * The autoloader expects to receive a fully-namespaced resource name as an
     ...
     * @return boolean True if the given resource was loaded
	 */
	 ...
	 * that namespace, skipping any that aren't and leaving them for another
	 ...
	 * by replacing the namespace separator with the directory separator,
	 ...
	 * @param string $separator The character(s) to treat as the namespace separator
